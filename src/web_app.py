...
""",
    ...
    with st.sidebar.expander("高级选项", expanded=False):
        ...
        maturity_hits = st.slider("轨迹成熟帧", 3, 12, 6, key="maturity_hits")
        gate_min_frames = st.slider("闸门停留帧", 1, 5, 2, key="gate_min_frames")
        reconnect_max_age = st.slider("断轨补偿帧", 15, 90, 60, key="reconnect_max_age")
    ...
    with main_right:
        ...
        kv_row("轨迹成熟帧", str(maturity_hits))
        kv_row("闸门停留帧", str(gate_min_frames))
        ...
        if show_preview:
            ...
            if ok and preview_width > 0 and preview_height > 0:
                if orientation == "horizontal":
                    ...
                    gate_band = max(4, int(max(5, preview_height // 100) * band_margin_scale))
                    cv2.line(preview_frame, (0, line_value - gate_band), (preview_width, line_value - gate_band), (0, 255, 255), 3)
                    cv2.line(preview_frame, (0, line_value + gate_band), (preview_width, line_value + gate_band), (0, 255, 255), 3)
                    cv2.line(preview_frame, (0, line_value), (preview_width, line_value), (0, 180, 255), 1)
                else:
                    line_value = int(preview_width * line_position / 100)
                    gate_band = max(4, int(max(5, preview_width // 100) * band_margin_scale))
                    cv2.line(preview_frame, (line_value - gate_band, 0), (line_value - gate_band, preview_height), (0, 255, 255), 3)
                    cv2.line(preview_frame, (line_value + gate_band, 0), (line_value + gate_band, preview_height), (0, 255, 255), 3)
                    cv2.line(preview_frame, (line_value, 0), (line_value, preview_height), (0, 180, 255), 1)
        ...
        try:
            with st.spinner("正在处理视频..."):
                ...
                stats = process_video(
                    ...
                    maturity_hits=maturity_hits,
                    gate_min_frames=gate_min_frames,
                    reconnect_max_age=reconnect_max_age,
                )
            ...
            with tabs[2]:
                browser_video_path = VIDEO_BROWSER_OUTPUT_PATH if os.path.exists(VIDEO_BROWSER_OUTPUT_PATH) and os.path.getsize(VIDEO_BROWSER_OUTPUT_PATH) > 0 else VIDEO_OUTPUT_PATH
                if os.path.exists(browser_video_path) and os.path.getsize(browser_video_path) > 0:
            ...
            with tabs[3]:
                ...
                browser_video_path = VIDEO_BROWSER_OUTPUT_PATH if os.path.exists(VIDEO_BROWSER_OUTPUT_PATH) and os.path.getsize(VIDEO_BROWSER_OUTPUT_PATH) > 0 else VIDEO_OUTPUT_PATH
                if os.path.exists(browser_video_path) and os.path.getsize(browser_video_path) > 0:
                    with open(browser_video_path, "rb") as video_file:
                        st.download_button(
                            label="下载处理后的视频",
                            data=video_file,
