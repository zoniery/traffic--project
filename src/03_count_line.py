...
maturity_hits = int(os.environ.get("TP_MATURITY_HITS", "6") or "6")
trace_length = int(os.environ.get("TP_TRACE_LENGTH", "20") or "20")
gate_min_frames = int(os.environ.get("TP_GATE_MIN_FRAMES", "2") or "2")
min_displacement_env = os.environ.get("TP_MIN_DISPLACEMENT")
min_displacement = int(min_displacement_env) if min_displacement_env else None
reconnect_max_age = int(os.environ.get("TP_RECONNECT_MAX_AGE", "60") or "60")
reconnect_distance_env = os.environ.get("TP_RECONNECT_DISTANCE")
reconnect_distance = int(reconnect_distance_env) if reconnect_distance_env else None
...
while cap.isOpened():
    ...
    gate_band = max(4, int(margin * band_margin_scale))
    if orientation == "horizontal":
        cv2.line(annotated_frame, (0, line_value - gate_band), (width, line_value - gate_band), (0, 255, 255), 2)
        cv2.line(annotated_frame, (0, line_value + gate_band), (width, line_value + gate_band), (0, 255, 255), 2)
        cv2.line(annotated_frame, (0, line_value), (width, line_value), (0, 180, 255), 1)
        tip = 'Above->Below = In, Below->Above = Out'
        tip_pos = (10, max(25, line_value - 10))
    else:
        cv2.line(annotated_frame, (line_value - gate_band, 0), (line_value - gate_band, height), (0, 255, 255), 2)
        cv2.line(annotated_frame, (line_value + gate_band, 0), (line_value + gate_band, height), (0, 255, 255), 2)
        cv2.line(annotated_frame, (line_value, 0), (line_value, height), (0, 180, 255), 1)
