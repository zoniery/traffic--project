...
VIDEO_BROWSER_OUTPUT_PATH = os.path.join(OUTPUT_DIR, "processed_video_h264.mp4")
...
def convert_video_for_browser(input_path: str, output_path: str = VIDEO_BROWSER_OUTPUT_PATH) -> str:
    """Transcode OpenCV output to browser-friendly H.264 MP4 when ffmpeg is available."""
...
def update_crossing_counts(
    ...
    lost_tracks=None,
    reconnect_max_age=60,
    reconnect_distance=None,
):
...
def process_video(
    ...
    while cap.isOpened():
        ...
        if orientation == "horizontal":
            cv2.line(annotated, (0, line_value - gate_band), (width, line_value - gate_band), (0, 255, 255), 2)
            cv2.line(annotated, (0, line_value + gate_band), (width, line_value + gate_band), (0, 255, 255), 2)
            cv2.line(annotated, (0, line_value), (width, line_value), (0, 180, 255), 1)
            tip = "Above->Below: In, Below->Above: Out"
            tip_pos = (10, max(25, line_value - 10))
        else:
            cv2.line(annotated, (line_value - gate_band, 0), (line_value - gate_band, height), (0, 255, 255), 2)
            cv2.line(annotated, (line_value + gate_band, 0), (line_value + gate_band, height), (0, 255, 255), 2)
            cv2.line(annotated, (line_value, 0), (line_value, height), (0, 180, 255), 1)
    ...
    cap.release()
    out.release()
    browser_video_path = convert_video_for_browser(VIDEO_OUTPUT_PATH)
