...
- `src/03_count_line.py`：读取 `data/raw_videos` 下的首个 mp4，按底边中心点 + Gate 状态机输出 `events.csv`
...
./.venv/bin/python -m pip install streamlit ultralytics supervision opencv-python pandas matplotlib imageio-ffmpeg torch torchvision torchaudio
```
...
.venv\Scripts\python -m pip install streamlit ultralytics supervision opencv-python pandas matplotlib imageio-ffmpeg torch torchvision torchaudio
```

### 3.4 统一运行入口

本项目统一使用仓库内的虚拟环境解释器：`.venv/bin/python`（Windows 为 `.venv\Scripts\python`）。为避免误用系统 `python3`，建议所有本地命令都通过根目录的 `run.py` 入口执行：

```bash
python3 run.py check
python3 run.py track
...
如果需要直接执行任意 Python 代码，也可以通过：

```bash
python3 run.py python -c "import sys; print(sys.executable)"
```
